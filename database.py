...
def create_support_ticket(telegram_id, telegram_name, phone, subject=""):
    ...
    cur.execute("""
        INSERT INTO support_tickets (
            ticket_number, telegram_id, telegram_name, phone,
            subject, status, created_at, closed_at
        )
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        "",
...
def get_support_ticket(ticket_number):
    ...
    cur.execute("""
        SELECT id, ticket_number, telegram_id, telegram_name,
               phone, subject, status, created_at, closed_at
...
def get_support_tickets_by_status(status='open', limit=30):
    create_tables()
