...
async def send_main_menu_like_vpn_layout(message: Message, greeting_text, action_text, banner_key=None, reply_markup=None, parse_mode="HTML"):
    """
    MAIN_MENU_VPN_STYLE_LAYOUT_FIX
    כמו בדוגמת VPN:
    1. הודעת שלום קצרה בנפרד.
    2. באנר נקי ללא caption.
    3. הודעת טקסט קצרה "בחר פעולה:" עם הכפתורים.

    חשוב: לא משתמשים כאן ב-widen_inline_screen_text,
    כדי שהבועה לא תתרחב/תישבר וכדי שהטקסט יוצג טבעי בצד ימין.
    """
    uid = message.from_user.id
...
async def start(message: Message):
    ...
    sent = await send_main_menu_like_vpn_layout(
        message,
